...
def maze_to_image(maze, block_size):
    ...
    for y, row in enumerate(maze):
        for x, cell in enumerate(row):
            if cell == 1:  # Wall
                draw.rectangle([x*block_size, y*block_size], fill=(0, 0, 0))
            elif cell == 2:  # Goal
                draw.rectangle([x*block_size, y*block_size], fill=(0, 255, 0))
            elif cell == 3:  # Player
                draw.rectangle([x*block_size, y*block_size], fill=(255, 0, 0))
