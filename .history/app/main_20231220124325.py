...
# Use a Streamlit session state to hold the current maze image
if 'current_maze_image' not in st.session_state:
    st.session_state.current_maze_image = generate_initial_maze()

st.title("NeuroMaze")
st.image(scale_image(st.session_state.current_maze_image), caption='Current Maze', use_column_width=False)
...
if action:
    ...
    # Update the current maze image
    st.session_state.current_maze_image = new_maze_image

    st.image(scale_image(new_maze_image), caption='Next Maze State', use_column_width=False)
